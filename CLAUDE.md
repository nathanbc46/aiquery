...
- **Auth-dependent `v-if`** ที่อาจต่างกันระหว่าง server/client ให้ครอบด้วย `<ClientOnly>` เสมอ (เช่น admin nav section).
- **ห้ามใช้ `ssr: false`** — มี bug กับ Nuxt 3.x บน Windows (Vite Node IPC socket error).
